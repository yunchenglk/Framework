...
namespace YunChengLK.Framework.Data.Core
{
    public static class DataReaderExtensions
    {
        ...
        public class EntityConverter<T> where T : class, IEntity, new()
        {
            ...
            {
                DynamicMethod dm = new DynamicMethod(string.Empty, typeof(IEnumerable<T>),
            ...
            private static void ReadInt32(ILGenerator il, LocalBuilder item,
                IList<EntityProperty> columnInfoes, LocalBuilder[] colIndices, int i)
            {
                il.Emit(OpCodes.Ldloc_S, item);
                il.Emit(OpCodes.Ldarg_0);
                ...
                IList<EntityProperty> columnInfoes, LocalBuilder[] colIndices, int i)
            {
                ...
                il.Emit(OpCodes.Ldloc_S, colIndices[i]);
                il.Emit(OpCodes.Callvirt, DataRecord_GetInt32);
                il.Emit(OpCodes.Call, columnInfoes[i].PropertyType.GetConstructor(
                    new Type[] { Nullable.GetUnderlyingType(columnInfoes[i].PropertyType) }));
